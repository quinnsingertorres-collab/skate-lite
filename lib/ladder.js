...
// The pattern a bus is running: its own trip's pattern if known, else the main one for its direction
export function patternFor(route, v) {
  if (!route) return null;
  if (v.pattern && route.patterns[v.pattern]) return route.patterns[v.pattern];
  const id = route.main?.[String(v.dir)];
  return (id && route.patterns[id]) || null;
}

export function variantLabel(v) {
  return v?.variant && v.variant !== "_" ? v.variant : "";
}

export function ladderPosition(route, v) {
  const pattern = patternFor(route, v);
...
// Minutes with one decimal, e.g. 452 s late -> "7.5 mins late"
export function adherenceMinutes(adherence) {
  return (Math.abs(adherence) / 60).toFixed(1);
}

export function adherenceLabel(adherence) {
  if (adherence == null) return null;
  const mins = adherenceMinutes(adherence);
  if (mins === "0.0") return "On time";
  return `${mins} mins ${adherence < 0 ? "early" : "late"}`;
}
